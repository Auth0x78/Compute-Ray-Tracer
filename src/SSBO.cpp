...
SSBO::SSBO(GLuint bindingLoc, GLuint usage, GLsizeiptr size, const void *data) {
  ...
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, bindingLoc,
                   m_id); // binding in shader
}

SSBO::~SSBO() { glDeleteBuffers(1, &m_id); }
...
void SSBO::BindBase() {
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, m_bindingLocation,
                   m_id); // binding = 0 in shader
}
