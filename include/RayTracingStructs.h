...
struct BVHNode {
  ...
  BVHNode()
      : boundsMin(glm::vec3(FLT_MAX)), boundsMax(glm::vec3(FLT_MIN)),
        startIndex(0), triangleCount(-1), _pad0(FP_NAN), _pad1(0), _pad2(0),
        _pad3(0) {}
};
...
struct Triangle {
  ...
  float _pad0; // offset 0
  glm::vec3 posB;
  float _pad1; // offset 16
  glm::vec3 posC;
  float _pad2; // offset 32

  glm::vec3 normA;
  float _pad3; // offset 48
  glm::vec3 normB;
  float _pad4; // offset 64
  glm::vec3 normC;
  float _pad5; // offset 80
               // stride = 96
};
__pragma(pack(pop))

    // === Compile-time checks ===
    static_assert(sizeof(RayTracingMaterial) == 64,
                  "RayTracingMaterial must be 64 bytes");
