...
public:
  ...
  void SetModelMatrix(const glm::mat4 &model) { modelMat = model; }

  const glm::mat4 &GetModelMatrix() { return modelMat; }
...
private:
  void Init(GLuint latDiv = 50, GLuint lonDiv = 50) {
    ...
    for (GLuint i = 0; i <= latDiv; ++i) {
      float theta =
          glm::pi<float>() * float(i) / float(latDiv); // Latitude angle [0, π]
      float sinTheta = sin(theta);
      float cosTheta = cos(theta);

      for (int j = 0; j <= lonDiv; ++j) {
        float phi = 2.0f * glm::pi<float>() * float(j) /
                    float(lonDiv); // Longitude angle [0, 2π]
    ...
    p_vertexBuffer =
        std::make_unique<VBO>(vertex.data(), vertex.size() * sizeof(vertex[0]));
    p_indicesBuffer = std::make_unique<EBO>(indices);

    p_vertexArray->LinkAttrib(*p_vertexBuffer, 0, 3, GL_FLOAT,
